...
export function useGlobalAuth() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (storedSession) {
                ...
                setProfile(parsedSession); // For this custom setup, session and profile are the same
            }
        } catch (error) {
        ...
        // Listen for storage events in case they log out in another tab
    ...
    // Also export a login and logout function for convenience
    const login = (userData: AuthProfile) => {
        localStorage.setItem('tanim_user_session', JSON.stringify(userData));
        setSession(userData);
        setProfile(userData);
        // Dispatch storage event so other components know immediately
