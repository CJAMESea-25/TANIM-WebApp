...
export async function createFarmer(farmerData: {
    ...
    const response = await apiPostAdmin<any[]>('/farmer', payload);
...
export async function updateFarmer(id: string, farmerData: Partial<{
    ...
    const response = await apiPatchAdmin<any[]>(`/farmer?farmer_id=eq.${id}`, payload);
    return response?.[0] || null;
}

export async function deleteFarmer(id: string) {
    return await apiDeleteAdmin<any>(`/farmer?farmer_id=eq.${id}`);
}
